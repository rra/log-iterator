...
# Helper function to strip the undef values out of a hash reference for easier
# comparison purposes.
#
# $hash - Reference to a hash
#
# Returns: The reference to the hash, also modified in place
sub strip_undef {
    my ($hash) = @_;
...
# stream read methods.  We don't care of a key exists but has an undef value.
for my $i (0 .. $#RESULT) {
    my $log_record = $RESULT[$i];
    is_deeply(strip_undef($stream->head), $log_record, "Head of line $i");
    is_deeply(strip_undef($stream->get),  $log_record, "Get of line $i");
}
...
my $parse = \&Log::Stream::Parse::Apache::Combined::_parse_timestamp;
for my $timestamp (sort keys %timestamp_to_time) {
    my $time = $timestamp_to_time{$timestamp};
    is($parse->(split(q{ }, $timestamp)), $time, "Parse of $timestamp");
}
## use critic
